...
QOQ_QUERY = (
    ...
    "    | project iif(Total==0, 0.0, round(100.0 * Onboarded / Total, 1)));\n"
    "print\n"
    "    Metrics = pack_array(\"Critical / High Alerts\",\"P1 / P2 Incidents\",\"Avg MTTR\",\"Risky Sign-ins\",\"Blocked Email Threats\",\"Device Onboarding\"),\n"
    "    Units   = pack_array(\"\",\"\",\"hrs\",\"\",\"\",\"%\"),\n"
    "    Dirs    = pack_array(\"Down\",\"Down\",\"Down\",\"Down\",\"Down\",\"Up\"),\n"
    "    ThisQs  = pack_array(todouble(a1),todouble(i1),todouble(m1),todouble(r1),todouble(e1),todouble(d1)),\n"
    "    LastQs  = pack_array(todouble(a2),todouble(i2),todouble(m2),todouble(r2),todouble(e2),todouble(d2))\n"
    "| mv-expand Metrics, Units, Dirs, ThisQs, LastQs\n"
    "| project Metric=tostring(Metrics), Unit=tostring(Units), GoodDir=tostring(Dirs), ThisQ=toreal(ThisQs), LastQ=toreal(LastQs)\n"
